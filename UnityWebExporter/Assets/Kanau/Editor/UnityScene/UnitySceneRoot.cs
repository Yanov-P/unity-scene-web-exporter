    ...
    public class UnitySceneRoot
    {
        ...
        void VisitToCreateMesh_r(GameObject go) {
            if (go.GetComponent<Renderer>()) {
                ...
                var matname = ComponentHelper.GetMaterialName(go);
                var gayname = matname + mesh.GetInstanceID().ToString();
                if (!containerTable.Contains<MeshContainer>(matname + mesh.GetInstanceID().ToString()))
                {
                    var c = new MeshContainer(mesh);
                    c.MaterialName = matname;
                    var resname = c.MaterialName + c.InstanceId;
                    containerTable.Add(c.MaterialName + c.InstanceId, c);
                }
            }
